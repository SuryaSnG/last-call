package com.last.call.gatewayservice.config;
...
public class DependencyHealthCheck {
    ...
    @Value("${ITEM_SERVICE_URL:http://localhost:8082}")
    private String itemServiceUrl;

    @Value("${ROOM_SERVICE_URL:http://localhost:8083}")
    ...
    public void checkDependencies() {
        waitForService(userServiceUrl + "/health", "User Service");
        waitForService(itemServiceUrl + "/health", "Item Service");
        waitForService(roomServiceUrl + "/health", "Room Service");
